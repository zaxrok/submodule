...
new (function() {
    ...
    ext.cs_temperature = function(port){
        if(device) {
            ...
            if(result[deviceNumber['temp']] == 0){
                result[deviceNumber['temp']] = 30; // init value
            }
        }
    ...
    ext.cs_geomagnetic = function(port, pos) {
        if(device) {
            ...
            if(result[deviceNumber['geom']] == 0){
                result[deviceNumber['geom']] = 1024; // init value
            }
        }
    ...
    ext.cs_light_sensor = function(port){
        if(device) {
            ...
            if(result[deviceNumber['light']] == 0){
                result[deviceNumber['light']] = 1024; // init value
            }
        }
    ...
    ScratchExtensions.register('CodeStar', descriptor, ext, {type: 'serial'});
})();